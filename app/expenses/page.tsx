...
export default function ExpensesPage() {
  ...
  const stats = useMemo(() => {
    ...
    // Monthly average (based on all time)
    ...
    // % of revenue (based on total sales revenue)
  ...
  const monthlyData = useMemo(() => {
    ...
    return months.map(mk => {
      ...
      return {
        ...
        Gastos: Math.round((byMonth[mk] || 0) * 100) / 100,
      };
    });
  }, [expenses]);
  ...
  return (
    <div className="space-y-6">
          ...
          <p className="text-[var(--text-secondary)] mt-1">
            ...
            {stats && (
              <span className="text-[var(--text-secondary)]">
                {' · '}{stats.expenseRatio.toFixed(1)}% de ingresos
              </span>
            )}
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {isFiltered && (
          <button
            onClick={() => { setCategoryFilter(''); setDateFrom(''); setDateTo(''); setSearch(''); }}
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr>
                  <th className="table-header cursor-pointer" onClick={() => toggleSort('date')}>
                    Fecha <SortIcon field="date" />
                  </th>
                  <th className="table-header cursor-pointer" onClick={() => toggleSort('description')}>
                  ...
                  <th className="table-header cursor-pointer text-right" onClick={() => toggleSort('amount')}>
                  ...
                  >
                    ...
                    <td className="table-cell">
                      <div>
                        ...
                        {expense.notes && (
                          <p className="text-xs text-[var(--text-secondary)] mt-0.5">{expense.notes}</p>
                        )}
                      </div>
                    </td>
                    <td className="table-cell">
                    ...
                    <td className="table-cell text-center">
                      {expense.isRecurring ? (
                        <span className="chip-yellow text-xs">
                          🔄 {expense.recurringInterval === 'mensual' ? 'Mensual'
                            : expense.recurringInterval === 'trimestral' ? 'Trimestral'
                            : 'Anual'}
                        </span>
                      ) : (
                    ...
                    <td className="table-cell text-right">
                      <button
                        ...
                        className="btn-ghost text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20"
                      >
                        Eliminar
                      </button>
                    </td>
              ...
              <tfoot>
                <tr className="bg-[var(--card-hover)] font-semibold">
                  ...
                  <td className="table-header text-right text-red-600 dark:text-red-400">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                <optgroup label="Marketing">
                  {EXPENSE_CATEGORIES.filter(c => ['Facebook Ads', 'Google Ads', 'TikTok Ads', 'Instagram Ads', 'Email Marketing', 'Influencers', 'Otro Marketing'].includes(c)).map(c => (
                    <option key={c} value={c}>{c}</option>
                  ))}
                </optgroup>
                <optgroup label="Operativos">
                  {EXPENSE_CATEGORIES.filter(c => ['Proveedores', 'Envíos', 'Material', 'Transporte', 'Comisiones'].includes(c)).map(c => (
                    <option key={c} value={c}>{c}</option>
                  ))}
                </optgroup>
                <optgroup label="Fijos">
                  {EXPENSE_CATEGORIES.filter(c => ['Suscripciones', 'Hosting', 'Impuestos', 'Oficina', 'Nóminas'].includes(c)).map(c => (
                    <option key={c} value={c}>{c}</option>
                  ))}
                </optgroup>
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                className="input-field"
                placeholder="Ej: Campaña Facebook Ads Enero"
                required
              />
            </div>
            <div>
            ...
            <div className="flex items-end pb-1">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={e => setForm(f => ({ ...f, isRecurring: e.target.checked, recurringInterval: e.target.checked ? 'mensual' : undefined }))}
            ...
            {form.isRecurring && (
              <div>
                ...
                <select
                  value={form.recurringInterval || 'mensual'}
                  onChange={e => setForm(f => ({ ...f, recurringInterval: e.target.value as 'mensual' | 'trimestral' | 'anual' }))}
                  className="select-field"
                >
