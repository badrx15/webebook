              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {sale.customerName && (
                      <span className="text-sm text-[var(--text-secondary)]">👤 {sale.customerName}</span>
                    )}
                  </div>
                  ...
                  {sale.notes && <p className="text-xs text-[var(--text-secondary)] italic">{sale.notes}</p>}
                </div>
                <div className="text-right sm:text-right flex sm:flex-col items-center sm:items-end gap-4 sm:gap-1">
                  <div>
                    <p className="text-sm text-[var(--text-secondary)]">Total</p>
                    <p className="text-xl font-bold text-green-600">{formatCurrency(sale.totalAmount, settings.currencySymbol)}</p>
                  </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.paymentMethod} onChange={e => setForm(f => ({ ...f, paymentMethod: e.target.value as PaymentMethod }))} className="select-field">
                {PAYMENT_METHODS.map(m => <option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            <div>
              <label className="label">Cliente (opcional)</label>
              <input type="text" value={form.customerName} onChange={e => setForm(f => ({ ...f, customerName: e.target.value }))} className="input-field" placeholder="Nombre del cliente" />
            </div>
            <div>
              <label className="label">Contacto (opcional)</label>
              <input type="text" value={form.customerContact} onChange={e => setForm(f => ({ ...f, customerContact: e.target.value }))} className="input-field" placeholder="Teléfono o email" />
            </div>
          ...
          <div>
            ...
            {products.length === 0 ? (
              <p className="text-sm text-[var(--text-secondary)] p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                No hay productos disponibles. Ve a Productos para crear uno.
              </p>
            ) : (
              <>
                <div className="flex flex-wrap gap-2 mb-3">
                  {products.map(p => (
                    <button key={p.id} type="button" onClick={() => addToCart(p.id)}
                      className="text-xs px-3 py-1.5 rounded-full border border-[var(--border)] hover:bg-blue-50 hover:border-blue-300 dark:hover:bg-blue-900/30 transition-colors">
                      + {p.name}
                    </button>
                  ))}
                </div>
                    ...
                    <table className="w-full text-sm">
                        ...
                        {cartItems.map(item => (
                          <tr key={item.productId}>
                            ...
                            <td className="table-cell text-center">
                              <div className="flex items-center justify-center gap-2">
                                <button type="button" onClick={() => updateCartQty(item.productId, item.quantity - 1)}
                                  className="w-7 h-7 rounded-full border border-[var(--border)] flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700">−</button>
                            ...
                            <td className="table-cell text-right">
                              <button type="button" onClick={() => removeFromCart(item.productId)} className="text-red-500 hover:text-red-700 text-xs">✕</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
                        <tr className="bg-[var(--card-hover)] font-semibold">
                          ...
                          <td className="table-cell text-right text-green-600">{formatCurrency(totals.totalAmount, settings.currencySymbol)}</td>
                          <td className="table-cell" />
                        </tr>
                        <tr className="bg-[var(--card-hover)] text-sm">
                          <td colSpan={3} className="table-cell text-right text-[var(--text-secondary)]">Beneficio estimado:</td>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="btn-primary" disabled={cartItems.length === 0}>Registrar Venta</button>
          </div>
        </form>
      </Modal>
    </div>
