...
export default function CheckoutContent() {
  ...
  const whatsappMessage = useMemo(() => {
    const itemsText = cartItems
      .map(item => `• ${item.productName} × ${item.quantity} — ${formatCurrency(item.totalPrice, currencySymbol)}`)
      .join('\n');

    const paymentLabel = paymentMethod === 'contrareembolso' ? 'Contra Reembolso' : 'Tarjeta';
    const orderShortId = orderId ? orderId.slice(0, 8) : '';

    const message = [
      `🐷 ¡Hola! Acabo de hacer un pedido en Ibéricos Gourmet`,
      '',
      `👤 ${fullName}`,
      `📞 ${phone}`,
  ...
  }, [cartItems, fullName, phone, street, city, province, postalCode, totalAmount, paymentMethod, orderId, currencySymbol]);

  const whatsappUrl = `https://wa.me/34614070656?text=${whatsappMessage}`;
        ...
        <div className="max-w-md w-full text-center bg-white rounded-3xl shadow-xl p-8 sm:p-12">
          ...
          <div className="space-y-3">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 w-full px-6 py-3 rounded-full bg-green-600 text-white font-semibold hover:bg-green-700 transition-all shadow-lg shadow-green-600/25"
            >
            ...
            <Link href="/" className="inline-flex items-center justify-center gap-2 w-full px-6 py-3 rounded-full bg-red-700 text-white font-semibold hover:bg-red-800 transition-all">Volver a la Tienda</Link>
          </div>
        </div>
      </main>
    );
