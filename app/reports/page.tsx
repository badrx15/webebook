...
export default function ReportsPage() {
  ...
  // Products ranking for this period
  ...
  const productChartData = useMemo(() =>
    productRanking.map(p => ({
      ...
      Beneficio: Math.round(p.profit * 100) / 100,
    })),
    [productRanking]
  );
  ...
  const expensesPieData = useMemo(() =>
    Object.entries(expensesChartData).map(([name, value]) => ({
      name,
  ...
  const dailyChartData = useMemo(() =>
    dailySummary.map(d => ({
      day: new Date(d.date).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' }),
      Ingresos: Math.round(d.revenue * 100) / 100,
      Beneficio: Math.round(d.profit * 100) / 100,
    })),
    [dailySummary]
  );
      ...
      <div ref={reportRef} className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="kpi-card">
            ...
            <span className="change text-[var(--text-secondary)]">
              {periodRevenue > 0 ? ((periodCost / periodRevenue) * 100).toFixed(1) : 0}% del ingreso
            </span>
          </div>
          <div className="kpi-card">
            ...
            <span className="change text-green-600 dark:text-green-400">
              {periodRevenue > 0 ? ((periodProfit / periodRevenue) * 100).toFixed(1) : 0}% margen
            </span>
          </div>
          <div className="kpi-card">
            <span className="label">Beneficio Neto</span>
            <span className={`value ${periodNetProfit >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400'}`}>
              {formatCurrency(periodNetProfit, settings.currencySymbol)}
            </span>
            <span className="change text-red-600 dark:text-red-400">
              Gastos: {formatCurrency(periodExpensesTotal, settings.currencySymbol)}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="card">
            <h3 className="text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-wider mb-4">
              Ranking de Productos {periodLabel}
            </h3>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center py-3 border-t-2 border-[var(--border)] mt-2">
                ...
                <span className={`text-base font-bold ${periodNetProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  {formatCurrency(periodNetProfit, settings.currencySymbol)}
                </span>
              </div>
              {periodRevenue > 0 && (
                <div className="flex justify-between items-center py-1">
                  ...
                  <span className="text-sm font-medium">
                    {((periodNetProfit / periodRevenue) * 100).toFixed(1)}%
                  </span>
                </div>
              )}
            </div>
                  ...
                  {dailySummary.map(d => (
                    <tr key={d.date} className="hover:bg-[var(--card-hover)] transition-colors">
                      ...
                      <td className="table-cell text-right">
                        <span className={d.revenue > 0 && (d.profit / d.revenue) * 100 >= 30 ? 'chip-green' : 'chip-yellow'}>
                ...
                <tfoot>
                  <tr className="bg-[var(--card-hover)] font-semibold">
                    ...
                    <td className="table-header text-right">
                      {periodRevenue > 0 ? ((periodProfit / periodRevenue) * 100).toFixed(1) : 0}%
                    </td>
                  </tr>
                </tfoot>
              </table>
  ...
  return (
    <div className={`flex justify-between items-center py-2 ${bold ? '' : 'border-t border-[var(--border)]'}`}>
      ...
      <span className={`text-sm ${bold ? 'font-bold' : 'font-semibold'} ${color}`}>
        {isNegative ? '-' : ''}{formatCurrency(Math.abs(value), currencySymbol)}
      </span>
    </div>
  );
}
